...
const MiniSearchProduct = ({ product}: miniSearchProductProps) => {
  ...
  return (
      <div className='grid grid-cols-[1fr_3fr] gap-5 items-center'>
          <div  className='w-full h-auto aspect-square cursor-pointer' onClick={handleAllSearchingBoxProcesses}>
              
          <Image  src={`${process.env.NEXT_PUBLIC_HOST}${product.attributes.thumbnail.data.attributes.url}`} alt='' className='w-full h-full object-contain'/>
          </div>
          <div className='grid grid-rows-2'>
              <div  className='cursor-pointer' onClick={handleAllSearchingBoxProcesses}>
                  <h1 className='text-xl font-semibold line-clamp-1'>{product.attributes.title }</h1>
              </div>
