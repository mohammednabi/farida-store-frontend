...
const MiniActiveFilter = ({
  filterTitle,
  filterName,
  deleteFilter,
}: MiniActiveFilterProps) => {
  ...
  return (
    <div className=" border-mainGray border-2 border-solid p-3 flex items-center gap-2 rounded-md">
      <h1 className="text-sm md:text-xl">{filterTitle}</h1>
      <IoMdClose
        className="text-sm md:text-xl text-mainPink cursor-pointer"
        onClick={deleteTheFilter}
      />
    </div>
  );
};
